...
func TestIsPodFailing(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "pending pod with no container status",
...
func TestPodsForReplicaSet(t *testing.T) {
	...
	pod1 := yamlToPod(t, testdata.PodReady)
	pod1.Name = "myapp-abc123-p1"

	pod2 := yamlToPod(t, testdata.PodReady)
	pod2.Name = "myapp-abc123-p2"

	// Unrelated pod — different owner
	unrelated := yamlToPod(t, testdata.PodReady)
	unrelated.Name = "other-pod"
	unrelated.OwnerReferences[0].UID = types.UID("other-rs")
